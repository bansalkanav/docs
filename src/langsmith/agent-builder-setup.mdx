...
Agent Builder can pull tools from one or more remote [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) servers. Configure MCP servers and headers in your workspace settings. Agent Builder automatically discovers tools and applies the configured headers when calling them.

See the [MCP Framework](/langsmith/agent-builder-mcp-framework#using-remote-mcp-servers) page for details on using remote MCP servers.
