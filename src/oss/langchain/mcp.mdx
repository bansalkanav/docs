...
You can use an interceptor to automatically append structured content to the tool message, making it visible to the model:
...
client = MultiServerMCPClient({...}, interceptors=[append_structured_content])
```
